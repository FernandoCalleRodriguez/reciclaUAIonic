...
export class FormularioDudaPage implements OnInit {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected temaService: TemaService,
                ...
                protected toastController: ToastController, protected config: ConfiguracionService) {
        this.tmax = 30;
        this.bmax = 1500;
    }
        ...
        if (this.formulario.valid) {
            ...
            if (this.edit) {
                this.dudaService.modificar(this.duda).subscribe(d => {
                    this.config.presentToast('Duda editada', 'success');
                    this.router.navigate(['/foro/duda/', d.Id]);
                }, error => {
                    this.config.presentToast('Error al editar la duda', 'danger');
                });
            } else {
                this.duda.Usuario_oid = this.usuario.Id;
                this.dudaService.crear(this.duda).subscribe(d => {
                    this.config.presentToast('Duda creada', 'success');
                    this.router.navigate(['/foro/duda/', d.Id]);
                }, error => {
                    this.config.presentToast('Error al crear la duda', 'danger');
                });
            }
        }
    }
